...
_SERIALIZABLE_CLASSES = _IR_CLASSES + _DOMAIN_CLASSES
_CLASS_BY_TAG = {c.__name__: c for c in _SERIALIZABLE_CLASSES}
...
def _point(p: OriginalPoint):
    return {"file": p.file, "char_offset": p.char_offset, "byte_offset": p.byte_offset, "line": p.line, "column": p.column}


def _span(s: OriginalSpan):
    return {"start": _point(s.start), "end": _point(s.end)}


def _encode(x):
    if isinstance(x, OriginalSpan):
        return {"$span": _span(x)}
    if isinstance(x, tuple):
        return [_encode(y) for y in x]
    if dataclasses.is_dataclass(x) and type(x) in _SERIALIZABLE_CLASSES:
...
def _decode(x):
    if isinstance(x, list):
        return tuple(_decode(y) for y in x)
    if isinstance(x, dict):
        if "$span" in x:
            return _decode_span(x)
        tag = x.get("tag")
        if tag is None:
            raise ArtifactVerificationError("object without explicit artifact tag")
        cls = _CLASS_BY_TAG.get(tag)
        if cls is None:
            raise ArtifactVerificationError(f"unknown artifact tag: {tag}")
        expected = {f.name for f in fields(cls)}
        actual = set(x) - {"tag"}
        if actual != expected:
            raise ArtifactVerificationError(f"wrong fields for {tag}: expected {sorted(expected)}, got {sorted(actual)}")
            ...
            if isinstance(v, tuple):
                for x in v:
                    if dataclasses.is_dataclass(x):
                        yield from _walk_ir(x)
            elif dataclasses.is_dataclass(v) and not isinstance(v, OriginalSpan):
                yield from _walk_ir(v)


def verify_ir(program: irm.IRProgram) -> None:
    if not isinstance(program, irm.IRProgram):
        raise ArtifactVerificationError("payload is not IRProgram")
    if program.ir_version != irm.IR_VERSION:
        raise ArtifactVerificationError("unsupported IR version")
    if not isinstance(program.principal, irm.IRAction):
        raise ArtifactVerificationError("principal is not an IR action")
    if not all(isinstance(x, irm.IRInitialFact) and isinstance(x.value, irm.IRValue) for x in program.initial_facts):
    ...
    if any(type(x) is not int or x <= 0 for x in serials):
        raise ArtifactVerificationError("identity serial must be a positive integer")
    if len(serials) != len(set(serials)):
        raise ArtifactVerificationError("duplicate identity serial")
    ...
    if any(s.kind not in {"place", "act", "role"} for s in program.symbols):
        raise ArtifactVerificationError("unknown semantic identity kind")
    for s in program.symbols:
        if s.kind == "role" and s.owner not in acts:
            raise ArtifactVerificationError("role owner is not an act")
        if s.kind != "role" and s.owner is not None:
            raise ArtifactVerificationError("non-role identity has owner")

    init_places = [x.place for x in program.initial_facts]
    if len(init_places) != len(set(init_places)) or set(init_places) != places:
        raise ArtifactVerificationError("invalid initial-fact/place structure")
    act_ids = [a.act for a in program.acts]
    if len(act_ids) != len(set(act_ids)) or set(act_ids) != acts:
        raise ArtifactVerificationError("invalid act-definition structure")

    for a in program.acts:
        if len(a.roles) != len(set(a.roles)):
            raise ArtifactVerificationError("duplicate role in act profile")
        for r in a.roles:
            if r not in roles or by[r].owner != a.act:
                raise ArtifactVerificationError("invalid role reference in act profile")

    for n in _walk_ir(program):
        if hasattr(n, "source_span") and not isinstance(n.source_span, OriginalSpan):
            raise ArtifactVerificationError("invalid source span")
        if isinstance(n, irm.IRNatural) and (type(n.value) is not int or n.value < 0):
            raise ArtifactVerificationError("negative/invalid Natural")
        if isinstance(n, irm.IRIndexValue):
            if n.side not in {"before", "zero", "after"} or type(n.magnitude) is not int or n.magnitude < 0:
                raise ArtifactVerificationError("invalid BidirectionalIndex value")
            if (n.side == "zero") != (n.magnitude == 0):
                raise ArtifactVerificationError("invalid BidirectionalIndex zero/magnitude relation")
        if isinstance(n, irm.IRSymbolValue) and not n.external_label:
        ...
        if isinstance(n, irm.IRReplaceCurrentFact) and not isinstance(n.value, irm.IRValue):
            raise ArtifactVerificationError("invalid replacement value kind")
        if isinstance(n, irm.IRPerformAct) and not all(isinstance(x, irm.IRRoleAssociation) for x in n.associations):
            raise ArtifactVerificationError("invalid role association kind")
        if isinstance(n, irm.IRProduceResult) and not isinstance(n.value, irm.IRValue):
            raise ArtifactVerificationError("invalid result value kind")
        if isinstance(n, irm.IRThen) and (not n.actions or not all(isinstance(x, irm.IRAction) for x in n.actions)):
            raise ArtifactVerificationError("invalid explicit sequence")
        if isinstance(n, irm.IRConditional) and (
            not isinstance(n.proposition, irm.IRProposition)
            or not isinstance(n.if_holds, irm.IRAction)
            or not isinstance(n.if_not, irm.IRAction)
        ):
            raise ArtifactVerificationError("invalid conditional semantic kind")
        if isinstance(n, irm.IRFixedRecurrence) and (
        ...
        if isinstance(n, irm.IRPostActionRecurrence) and (
            ...
            raise ArtifactVerificationError("invalid post-action recurrence")

    try:
        validate_canonical_ir(program)
    except CanonicalIRValidationError as e:
        raise ArtifactVerificationError(
            f"canonical IR semantic validation failed: {e.issue.code}: {e.issue.detail}"
        ) from e
...
def verify_artifact(data: bytes, *, expected_language_edition: str = LANGUAGE_EDITION) -> irm.IRProgram:
    try:
        ...
        raise ArtifactVerificationError("artifact is not canonical JSON data") from e
    if not isinstance(obj, dict) or set(obj) != {
        "artifact_version", "ir_version", "language_edition", "payload_sha256", "program"
    }:
        raise ArtifactVerificationError("artifact top-level fields invalid")
    if obj["artifact_version"] != ARTIFACT_FORMAT_VERSION:
        raise ArtifactVerificationError("unknown artifact version")
    if obj["ir_version"] != irm.IR_VERSION:
        raise ArtifactVerificationError("unknown IR version")
    if obj["language_edition"] != expected_language_edition:
        raise ArtifactVerificationError("incompatible language edition")
    if hashlib.sha256(_canonical(obj["program"])).hexdigest() != obj["payload_sha256"]:
        raise ArtifactVerificationError("artifact payload digest mismatch")
    try:
    ...
    if not isinstance(program, irm.IRProgram):
        raise ArtifactVerificationError("artifact payload is not IRProgram")
    verify_ir(program)
    return program


__all__ = [
    "ARTIFACT_FORMAT_VERSION", "ArtifactVerificationError", "artifact_dict",
    "serialize_artifact", "verify_ir", "verify_artifact",
]
